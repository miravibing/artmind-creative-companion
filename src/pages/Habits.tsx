...
export default function Habits() {
  ...
  const toggleHabit = async (id: string) => {
    ...
    if (!habit.completed) {
      ...
      setHabits((prev) => prev.map((h) => h.id === id ? { ...h, completed: false, streak: newStreak } : h));
    }
  };
  ...
  const addHabit = async () => {
    ...
    setHabits((prev) => [...prev, { id: data.id, name: data.name, icon: data.icon, streak: 0, completed: false }]);
    setNewHabitName("");
